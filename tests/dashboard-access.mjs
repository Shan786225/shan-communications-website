...
p=await hr.client.request('edit.php?id='+job);const edit={csrf:token(p),revision:token(p,'revision'),full_name:'QA job edited',email:'job@example.invalid',phone:'0123456789',role_name:'QA role',experience:'QA',availability:'QA',message:'QA fixture edited'};
...
let msg=await admin.request('messages.php?user='+hr.id);const nonce=token(msg,'nonce');const body={csrf:token(msg),nonce,message:'QA private thread <script>alert(1)</script>'};
assert.equal((await admin.request('messages.php?user='+hr.id,body)).status,303);assert.equal((await admin.request('messages.php?user='+hr.id,body)).status,303);
msg=await hr.client.request('messages.php?user=1');assert.match(msg.text,/QA private thread &lt;script&gt;/);assert.equal((msg.text.match(/QA private thread/g)||[]).length,1,'duplicate message not duplicated');
